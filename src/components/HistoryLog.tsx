...
export function HistoryLog({ propertyId }: { propertyId: string }) {
  ...
  if (!mine.length) return <p className="text-label text-dm-dim">No changes yet.</p>;
  const shown = all ? mine : mine.slice(0, 10);
  return (
    <div>
      <ul className="divide-y divide-dm-border/70">
        {shown.map((a) => (
          <li key={a.id} className="flex items-baseline justify-between gap-4 py-2 text-label">
            <span className="min-w-0 break-words">{a.text}</span>
            <span className="shrink-0 text-label text-dm-dim">{stamp(a.at)}</span>
          </li>
        ))}
      </ul>
      {mine.length > 10 && (
        <button className="mt-2 text-label text-dm-blue hover:underline" onClick={() => setAll((v) => !v)}>
