...
import { Building2, Landmark, Pencil, Users } from 'lucide-react';
    ...
    <article className="rounded-xl border border-dm-border bg-dm-surface">
      <header className="flex items-start justify-between gap-3 border-b border-dm-border p-5">
        <div className="flex items-start gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-dm-blue/30 bg-dm-blue/10 text-dm-blue">
            <Landmark className="h-4 w-4" />
          </div>
          <div>
            <h2 className="text-lg font-semibold leading-snug">{e.name}</h2>
            <div className="mt-1 flex flex-wrap items-center gap-2">
              <span className="rounded border border-dm-blue/30 bg-dm-blue/10 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider text-dm-blue">{entityTypeLabel[e.entityType]}</span>
              <span className="font-mono text-[11px] text-dm-dim">{e.jurisdiction}</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <dl className="grid grid-cols-3 gap-4 border-b border-dm-border p-5">
        <div>
          ...
          <dd className="mt-1 font-mono text-sm">{e.sosId ?? '—'}</dd>
        </div>
        <div>
          <dt className="label">Formed</dt>
          <dd className="mt-1 font-mono text-sm">{e.formationDate ? fmtDate(e.formationDate) : '—'}</dd>
        </div>
        <div>
          <dt className="label">Deeds in / out</dt>
          <dd className="mt-1 font-mono text-sm">{acquired} / {sold}</dd>
        </div>
      </dl>
      ...
      <div className="space-y-5 p-5">
        <div>
          <div className="mb-2 flex items-center justify-between">
            <span className="label flex items-center gap-1.5"><Users className="h-3 w-3" /> Registered agent / managers</span>
            {!editing && (
        ...
        <div>
          <div className="label mb-2 flex items-center gap-1.5"><Building2 className="h-3 w-3" /> Associated properties ({linked.length})</div>
          {linked.length ? (
              ...
              {linked.map((p) => (
                <li key={p.id}>
                  <button onClick={() => openProperty(p.id)} className="flex w-full items-center justify-between gap-3 rounded-md border border-transparent px-3 py-2 text-left text-sm transition-colors hover:border-dm-border hover:bg-dm-hover">
                    <span>{p.name}</span>
                    <span className="font-mono text-[11px] text-dm-dim">{p.address.city}, {p.address.state}</span>
                  </button>
                </li>
              ))}
            </ul>
